...
\end{figure}
We can see that $\boldsymbol{d}=(2.7, 2.4)$ and we need to split in x-axis for our root node. The split value is then $(0.6*0.5) + (3.3*0.5)=1.95$. And we can partition primitives in the left and right node as:
\begin{figure}[H]
	\centering
	\captionsetup{justification=centering}
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\If{$root$ $\rightarrow$ $AABB$ $\rightarrow$ $intersect(ray, t_{min}, t_{tmax}) = false$}
			\State $return\;\;false$
		\EndIf
...
\hline\hline                                      %inserts double horizontal lines
& Custom scene 1 & Custom scene 2\\ [0.5ex] % inserts table heading 
\hline
...
\\
C.G build(s)& $(\rho=4)$\:13.276 s, $(\rho=6)$\:23.271 s, $(\rho=8)$\:23.859 s& $(\rho=8)$\:11.22 s \\
\\
\hline \hline
\\
BVH\_C render(s)& 100.15 s & 663.84 s \\
\\
C.G render(s)&  $(\rho=4)$\:218.15 s, $(\rho=6)$\:183.961 s, $(\rho=8)$\:164.468 s& $(\rho=8)$\:2076.93 s \\
\\
\hline \hline
\\
BVH\_C avg \# isect tests / isect ray& 320.96 & 79.6917\\
\\
C.G avg\# isect tests / isect ray& $(\rho=4)$\:560.504, $(\rho=6)$\:466.015, $(\rho=8)$\:417.912& $(\rho=8)$\:264.514 \\
\\
\hline \hline
    \end{tabular}
}
